...
namespace CrouMetro.Core.Managers
{
    public class SpreadManager
    {
        ...
        public static async Task<HttpStatusCode> DestroySpread(long id, bool? trim, UserAccountEntity userAccountEntity)
        {
            var param = new Dictionary<String, String>();
            if (trim == true) param.Add("trim_user", trim.ToString());
            var theAuthClient = new HttpClient();
            var request = new HttpRequestMessage(HttpMethod.Post, (String.Format(EndPoints.SPREAD_DESTORY, id)));
        ...
        public static async Task<List<PostEntity>> getSpreadHtmlParse(int offSet, String screenName, bool? trim,
            UserAccountEntity userAccountEntity)
        {
            ...
            var requestMsg = new HttpRequestMessage(new HttpMethod("GET"),
                "https://croudia.com/spreads/spreadia/" + screenName + "?offset=" + offSet);
