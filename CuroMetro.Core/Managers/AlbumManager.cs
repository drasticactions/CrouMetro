...
namespace CrouMetro.Core.Managers
{
    public class AlbumManager
    {
        ...
        private static string RemoveHtmlComments(string input)
        {
            ...
            return (from s in temp let str = string.Empty select !s.Contains("-->") ? s : s.Substring(s.IndexOf("-->", StringComparison.Ordinal) + 3) into str where str.Trim() != string.Empty select str).Aggregate(output, (current, str) => current + str.Trim());
        }
